...
module.exports = async function sharpVinyl (file, config, options) {
  ...
  const image = file.contents ? sharp(file.contents) : sharp(file.path)
  ...
  try {
    ...
    const detail = error.message || error.stack || JSON.stringify(error) || String(error)
    flog(PLUGIN_NAME + ':', chalk.red('Sharp error detail: ' + detail))
    flog(PLUGIN_NAME + ':', chalk.red('Error constructor: ' + (error.constructor && error.constructor.name)))
    flog(PLUGIN_NAME + ':', chalk.red('Error keys: ' + Object.keys(error).join(', ')))
    throw new PluginError(PLUGIN_NAME, errPrefix + detail, { showStack: true })
  }
}
